...
bool UDreamInteractionTargetResolver::ResolveScreenTarget(
	APlayerController& PlayerController, EDreamInteractionMode Mode, FDreamInteractionTarget& OutTarget) const
{
	...
	return ResolveWorldRay(*PlayerController.GetWorld(), WorldOrigin, WorldOrigin + WorldDirection * TraceDistance,
		Mode, OutTarget, PlayerController.GetPawn());
}

bool UDreamInteractionTargetResolver::ResolveWorldRay(UWorld& World, const FVector& Start, const FVector& End,
	EDreamInteractionMode Mode, FDreamInteractionTarget& OutTarget, const AActor* IgnoredActor) const
{
	OutTarget = FDreamInteractionTarget();
